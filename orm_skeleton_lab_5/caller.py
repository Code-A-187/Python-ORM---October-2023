...
def find_books_by_genre_and_language(book_genre: str, book_language: str):   # Solution 1
    return Book.objects.filter(genre=book_genre, language=book_language)


def find_authors_nationalities():                           # Solution 2
...
def order_books_by_year():                                  # Solution 3
...
def delete_review_by_id(reviewer_id: int):                    # Solution 4
    reviewer = Review.objects.get(id=reviewer_id)
    reviewer.delete()
    return f'Review by {reviewer.reviewer_name} was deleted'


def filter_authors_by_nationalities(nationality: str):         # Solution 5
    authors = (
...
def filter_authors_by_birth_year(year_1, year_2):                # Solution 6
    authors = Author.objects.filter(
        birth_date__year__range=(year_1, year_2)
    ).order_by('-birth_date')
...
def change_reviewer_name(reviewer_name: str, new_name: str):       # Solution 7
    Review.objects.filter(
        reviewer_name=reviewer_name).update(reviewer_name=new_name)
